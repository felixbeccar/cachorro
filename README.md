...
- **Today tab** — the live session you use in the gym.
  - **Setup screen**: if nothing's planned or started yet today, you get an open question instead of a pre-built routine — "Good morning, Felix — what do you want to train today?" with checkboxes for legs, core, chest, back, arms, shoulders (leave everything unchecked for full body). Tap **Build session** and the routine generates around only what you picked, or tap the mic below it and just say what you want — "legs and core, 45 minutes" — same voice parsing as everywhere else in the app.
  - Once a session exists (built here, planned from the Plan tab, or resumed from earlier today), you get the exercise list: a voice command bar and a session report card at the top, then the exercises, in a ~40-45 min routine covering every major muscle group. It favors exercises you haven't done, or haven't done in a while, over ones you keep repeating, and avoids exactly repeating last session's picks.
